...
type CorrectnessCheck a
  = ReaderT
      CheckEnv
      (ReaderT ErrorContext (StateT CheckState (Writer [CheckError])))
      a
...
checkCodeBlock block = forM_ block $ \statement -> work statement
  where
...
check (Assignment name rhs) =
  withContext ("in assignment " ++ P.render (printAst (Assignment name rhs))) $ do
    check rhs
    addVar name
check (FunctionCall name args) =
  withContext ("in function call " ++ name ++ "()") $ do
    expectedArityMaybe <- lookupFuncArity name
    varExists <- checkVarExists name
    case expectedArityMaybe of
      Nothing ->
        if varExists
          then logError (NotAFunction name)
          else logError (UndefinedFunction name)
...
prettyPrintErrors errs =
  forM_ errs $ \err -> do
    ...
    forM_ (reverse $ errorContext err) $ \ctx -> putStrLn $ "  .. " ++ ctx
...
doCorrectnessCheck ast =
  let (env, redefinitionErrors) = collectFunctions ast
      remainingErrors =
        execWriter
          (runStateT
             (runReaderT (runReaderT (mapM_ check ast) env) [])
             Set.empty)
